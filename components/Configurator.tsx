...
const brochures = [
  { model: "BALI CATSMART", pdf: publicAsset("/brochures/bali-catsmart-katalog-premium-pl.pdf"), cover: publicAsset("/images/brochures/bali-catsmart-cover-01.jpg"), pages: 28 },
  { model: "BALI CATSPACE", pdf: publicAsset("/brochures/bali-catspace-katalog-premium-pl.pdf"), cover: publicAsset("/images/brochures/bali-catspace-cover-01.jpg"), pages: 28 },
  { model: "BALI 4.2", pdf: publicAsset("/brochures/bali-42-katalog-premium-pl.pdf"), cover: publicAsset("/images/brochures/bali-4-2-cover-01.jpg"), pages: 28 },
  { model: "BALI 4.3", pdf: publicAsset("/brochures/bali-43-katalog-premium-pl.pdf"), cover: publicAsset("/images/brochures/bali-4-3-cover-01.jpg"), pages: 28 },
  { model: "BALI 4.6", pdf: publicAsset("/brochures/bali-46-katalog-premium-pl.pdf"), cover: publicAsset("/images/brochures/bali-4-6-cover-01.jpg"), pages: 28 },
  { model: "BALI 5.2", pdf: publicAsset("/brochures/bali-52-katalog-premium-pl.pdf"), cover: publicAsset("/images/brochures/bali-5-2-cover-01.jpg"), pages: 28 },
  { model: "BALI 5.8", pdf: publicAsset("/brochures/bali-58-katalog-premium-pl.pdf"), cover: publicAsset("/images/brochures/bali-5-8-cover-01.jpg"), pages: 28 },
  { model: "BALI 7.0", pdf: publicAsset("/brochures/bali-70-katalog-premium-pl.pdf"), cover: publicAsset("/images/brochures/bali-7-0-cover-1.jpg"), pages: 28, release: "PREMIERA 2027" },
];
...
export function Configurator() {
  ...
  const createPdfBlob = async (historyItem?: HistoryOffer) => {
    ...
    if (!historyItem || payload) {
      ...
      priceRows.push([model.excellencePackage.name, { text: money(calculation.excellence ?? 0), alignment: "right" }]);
    ...
    <main className={dark ? "app dark" : "app light"}>
      ...
      {step === 0 ? (
        ...
        <>
          ...
          {step === 3 && <section className="content-stage equipment-stage">
            <SectionHead eyebrow="Krok 3" title="Wyposażenie dodatkowe" text={`${allOptions.length} pozycji z cennika ${model.name}. Wybierz to, co definiuje Twój styl podróżowania.`}/>
            {editingOfferNumber && <div className="edit-offer-banner"><span>EDYTUJESZ OFERTĘ</span><strong>{editingOfferNumber}</strong><button type="button" onClick={startNewOffer}>Anuluj edycję</button></div>}
            <div className="equipment-layout">
              <aside className="filters"><label>WYSZUKAJ<input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="np. generator, Raymarine…"/></label><label>KATEGORIA<select value={category} onChange={(e) => setCategory(e.target.value)}>{categories.map((item) => <option key={item}>{item}</option>)}</select></label><label>CENA DO <b>{money(maxPrice)}</b><input type="range" min="1000" max="250000" step="1000" value={maxPrice} onChange={(e) => setMaxPrice(Number(e.target.value))}/></label><button className="ghost" onClick={() => { setSearch(""); setCategory("Wszystkie"); setMaxPrice(250000); }}>Wyczyść filtry</button></aside>
              <div className="option-list"><div className="results"><span>{filteredOptions.length} wyników</span><span>{chosenOptions.length} wybranych</span></div>{filteredOptions.map((item) => {
                const key = selectionKey(item);
